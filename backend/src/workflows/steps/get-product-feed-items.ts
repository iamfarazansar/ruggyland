      ...
      for (const product of products) {
        ...
        const salesChannel = product.sales_channels?.find((channel: any) => {
          return channel?.stock_locations?.some((location: any) => {
            return (
              location?.address?.country_code?.toLowerCase() === countryCode
            );
          });
        });
        ...
        for (const variant of product.variants) {
          ...
          feedItems.push({
            ...
            price: formatPrice((originalPrice as number) || 0, currencyCode),
            sale_price: salePrice
              ? formatPrice(salePrice as number, currencyCode)
              : undefined,
